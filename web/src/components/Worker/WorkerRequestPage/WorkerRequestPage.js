                ...
                {clients.map(client=>
                    <div className="rectangle" onClick={(e)=>{viewProfile(e,client)}}>
                        ...
                        <button class="acceptbutton" onClick={()=>{
                            fetch(" https://f9a1-37-111-214-100.in.ngrok.io/payment/initialize-payment",{method:"POST", headers:{"content-type":"application/json"} ,body:JSON.stringify({
                                "total_amount":100,
                                "currency":"BDT",
                                "cus_name":"Zhalok",
                                ...
                                console.log(data)
                                window.location.replace(data.redirectGatewayURL)}).catch(e=>console.log(e))
                        }} >Buy</button>
                        <button class="declinebutton" onClick={()=>{decline(client); removeAfterAcceptOrDecline(client.name)}}>Request</button>
                    </div>
                )}
            </div>
            <Footer/>
